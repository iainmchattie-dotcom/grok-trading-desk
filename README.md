...
watches every launch as it happens. Pure metric filtering: liquidity band, holder
count, top-10 concentration, dev holding, age window, buy count, buy/sell ratio,
mint authority. Nothing downstream costs anything until a token clears this gate,
which is why the gate is deliberately narrow.

**2 · auditor** (`crypto/auditor.py`, grok-4-fast) — Audits the wallet graph behind
...
**3 · narrative** (`crypto/narrative.py`, grok-4-fast) — Rates meme potential: is
...
**4 · screener** (`stocks/screener.py`, code) — Once per trading day. Pulls the
day's movers from Alpaca and filters on price band, average volume, market-cap
band, relative volume, gap size (absolute — a gap down is as tradeable as a gap up)
and excluded sectors. Survivors are ranked by relative volume.

**5 · analyst** (`stocks/analyst.py`, grok-4-fast) — Both halves of the equity
...
**6 · radar** (`stocks/radar.py`, grok-4-fast) — Two weeks of news and sentiment:
...
**7 · insider** (`stocks/insider.py`, grok-4-fast) — Form 4 and 13F flow.
...
**8 · crypto_pulse** (`crypto/crypto_pulse.py`, grok-4-fast, 15-min cache) — The
...
**9 · market_pulse** (`stocks/market_pulse.py`, grok-4-fast, 30-min cache) — The
...
**10 · allocator** (`shared/allocator.py`, grok-4-fast) — Once a day, splits the
...
**11 · crypto_checker** (`crypto/crypto_checker.py`, **grok-4**) — The adversarial
...
**12 · stock_checker** (`stocks/stock_checker.py`, **grok-4**) — Same job on the
...
**13 · exit_manager** (`shared/exit_manager.py`, grok-4-fast) — Every 4 hours over
...
| `grok` | API key, base URL, `fast_model` / `full_model`, timeout, retries |
...
| `crypto_filter` | Scout thresholds |
...
| `logging` | JSONL path, stdout echo |
...
`all_agent_scores` carries every sub-bot's full output for the trade, which is what
makes a bad entry diagnosable after the fact: you can see which bot was wrong.
